...
// Kuula: the stock crate draws the seed from `arc4random` (macOS) or
// `getrandom` (Linux) here and only falls back to the C `luaL_makeseed`
// elsewhere, so a Lua state's string hash, and with it `pairs` order
// over string keys, was random per state on those platforms while
// Windows got the fixed seed `.cargo/config.toml` pins through
// `luai_makeseed`. Every platform takes the C path now; this is the
// only change in the vendored copy.
